...
function ProductOptionsModal({
  ...
  const defaultType =
    ["سكالوب", "ميكس", "????"].includes(item.type)
      ? item.type
      : detectedFilling || "??????";
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`?الطلب مجهز ${item.name}`}
        onClick={(event) =>
...
export default function App() {
  ...
  async function loginUnified(event) {
    ...
    if (!supabase) {
      setAuthLoginError("???? الطلب مجهز ??? الطلب مجهز.");
      return;
    }
    ...
    if (!sessionToken) {
      setAuthLoginError("تعذر إنشاء جلسة الدخول.");
      return;
    }
  ...
  async function checkoutOrder({
    ...
    const message = [
      ...
      `???????: ${customerAddress}`,
      "",
      `?الطلب مجهز ???? ف? ??? ?? ${storeConfig.name}`,
      "",
      "?ف???? ?????:",
      ...lines,
      "",
      `الطلب مجهز??: ${formatPrice(
        subtotal
      )}`,
      `???حذف المنتج?: ${formatPrice(
        delivery
      )}`,
      `??الطلب مجهز?: ${formatPrice(
        total
