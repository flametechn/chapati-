...
export default function App() {
  ...
  async function loginCustomer(event) {
    ...
    if (!supabase) {
      setCustomerLoginError(
        "خدمة تسجيل الحساب غير متاحة حالياً."
      );
      return;
    }
  ...
  async function loginUnified(event) {
    ...
    if (!identifier) {
      setAuthLoginError("يرجى إدخال رقم الهاتف أو اسم المستخدم.");
      return;
    }
    ...
    if (
      ...
      setAuthLoginError("هذا الحساب غير صالح.");
      return;
    }
    ...
    const id =
      ...
      result.id ||
      null;
      setAuthLoginError("هذا الحساب لا يملك صلاحية الدخول إلى هذه الواجهة.");
    const name =
      account.name ||
      result.owner_name ||
      result.driver_name ||
  ...
  async function saveAuthDriverPhone(event) {
    ...
    if (
      ...
      setAuthPhoneError(
        "جلسة السائق غير صالحة. أعد تسجيل الدخول."
      );
      return;
    }
    ...
    if (!authSessionToken) {
      setAuthPhoneError(
        "جلسة السائق منتهية. أعد تسجيل الدخول."
      );
      return;
    }
  ...
  async function checkoutOrder({
    ...
    const message = [
      ...
      ...lines,
      "",
      `مجموع المنتجات: ${formatPrice(
        subtotal
      )}`,
      `مصاريف التوصيل: ${formatPrice(
        delivery
      )}`,
          ...
          >
            <div className="customer-login__header">
              <div>
                ...
                <p>
                  {customer
                    ? "معلومات حسابك محفوظة ويمكنك استخدامها عند الطلب."
                    : "أنشئ حسابك بإدخال الاسم ورقم الهاتف."
                </p>
              </div>
              ...
              <div>
                ...
                <p>
                  {authStep === "dashboard"
                    ...
                    : authStep === "phone"
                        ? "أدخل رقم هاتفك لإكمال بيانات السائق."
