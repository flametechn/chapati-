...
export default function OwnerDashboard({
  ...
  const [driverDeleting, setDriverDeleting] = useState(null);
  ...
  async function createDriver(event) {
    ...
    if (name.length < 2) {
      setError("اسم السائق يجب أن يكون حرفين على الأقل.");
      return;
    }
    ...
    try {
      ...
      if (!data?.success) {
        const errorMessages = {
          invalid_or_expired_session: "الجلسة غير صالحة أو منتهية الصلاحية.",
      ...
      setMessage("تم إنشاء السائق بنجاح.");
            ...
            {drivers.length === 0 ? (
              ...
              drivers.map((driver) => (
                ...
                >
                  ...
                  <button type="button" className="owner-driver__delete" onClick={() => deleteDriver(driver)} disabled={driverDeleting === driver.id}>{driverDeleting === driver.id ? "\u062c\u0627\u0631\u064a \u0627\u0644\u062d\u0630\u0641..." : "\u062d\u0630\u0641"}</button>
                </article>
              ))
            )}
          </div>
