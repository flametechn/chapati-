...
function ProductOptionsModal({ item, onClose, onConfirm }) {
  ...
  if (isSpecial) {
    ...
    selectedOptions.push({
      ...
      price: specialChoice === "cheese" ? 0 : 50,
    });
  }
      ...
      >
            ...
            <div className="product-options__choices">
              ...
              >
                <input
                  ...
                  onChange={(event) =>
                    setExtraFilling(event.target.checked)
                  }
                />
        ...
        {fillingType && (
          <div className="product-options__section">
            ...
            <div className="product-options__choices">
              {freeExtraOptions.map((option) => {
                const selected = freeExtras.includes(option.id);
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      checked={selected}
                      onChange={() =>
                        toggleFreeExtra(option.id)
                      }
                    />

                    <span>{option.name} مجاني</span>
                  </label>
                );
              })}
            </div>
...
function CartModal({
  ...
  const delivery =
    cart.length > 0 ? storeConfig.deliveryFee : 0;
...
export default function App() {
  ...
  const [ownerPasswordInput, setOwnerPasswordInput] =
    useState("");
  const [ownerOrdersToday, setOwnerOrdersToday] =
    useState(null);
  const [ownerCheckLoading, setOwnerCheckLoading] =
    useState(false);
  const [ownerCheckError, setOwnerCheckError] =
    useState("");
  const [ownerBoxDismissed, setOwnerBoxDismissed] =
    useState(false);
  ...
  const [isCustomerLoginOpen, setIsCustomerLoginOpen] =
    useState(false);

  const [customerPhoneInput, setCustomerPhoneInput] =
    useState("");

  const [customerNameInput, setCustomerNameInput] =
    useState("");

  const [customerLoginLoading, setCustomerLoginLoading] =
    useState(false);

  const [customerLoginError, setCustomerLoginError] =
    useState("");
  ...
  async function checkOwnerPhone(event) {
    ...
    const normalizedOwnerPhone = ownerPhoneInput
      .trim()
      .replace(/^0/, "213");
      ...
      {!ownerBoxDismissed && (
        <div className="owner-check">
          {ownerOrdersToday === null ? (
            <form
              className="owner-check__form"
              onSubmit={checkOwnerPhone}
            >
              <input
                type="tel"
                inputMode="tel"
