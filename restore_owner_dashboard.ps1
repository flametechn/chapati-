...
export default function OwnerDashboard() {
  ...
  async function handleLogin(event) {
    ...
    const { data, error: rpcError } = await supabase.rpc("owner_login", {
  ...
  async function assignDriver(order) {
    ...
    if (rpcError || !data?.success) {
      const errorMessages = {
        ...
        order_already_delivered: "هذا الطلب تم توصيله بالفعل.",
      };
...
Set-Content ".\src\OwnerDashboard.jsx" -Value $content -Encoding UTF8

Write-Host "OWNER_DASHBOARD_RESTORED"